...
for (k, title, cmap), a in zip(panels, ax.flat):
    ...
    a.clabel(cs, inline=True, fontsize=7, fmt="%.0f" if k in ("hhi", "persuasion_total") else "%.2f")
    a.set_title(title, fontsize=10)
    a.set_xlabel("Twin-buyer share α", fontsize=9)
    a.set_ylabel("Twin persuadability γ", fontsize=9)
...
ax[1].set_title(f"Total persuasion spend (α=γ=0 baseline ≈ {base:,.0f})", fontsize=9)

...
1. **$\gamma$ is stylized, not measured.** The single most valuable extension is an empirical estimate: run frontier shopping agents on matched product listings that differ only in agent-directed optimization, and estimate the choice-share shift per unit of persuasion. That converts P2 from an assumption into a prediction. The experimental designs in Allouah et al. (2026) and Salvi et al. (2026) are the natural templates.

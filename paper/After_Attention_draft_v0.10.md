...
*Draft v0.10 — September 19, 2026. Working paper; comments welcome.*

---

## Abstract

The consumer economy runs in part on *attention rent*: margin that sellers extract because human buyers search a little, remember imperfectly, and respond to persuasion. As personal purchasing agents ("twins") take over consumer choice, the standard expectation is that this rent collapses. We model a market with a mixed population of human and agent buyers in which sellers choose price, consumer-directed advertising, and agent-directed optimization spend, and we show that the collapse is neither smooth nor complete. Three results follow from the structure of agent demand. First, because a well-informed agent's demand is close to winner-take-most in quality-adjusted price, sellers face a discrete choice between a human-exploiting price and an agent-competitive one, which produces a *tipping point*: in our simulations gross margin falls 1.4 percentage points as the agent share rises from 0 to 0.2, then drops from 40% to 33% by a share of 0.3 and to 4% when all buyers are agents. Second, the mixed population admits no pure-strategy equilibrium in the interior — a discrete-grid counterpart of Varian (1980) — which we prove and which matches the numerics exactly: every convergent run in our sweeps sits at one of the two boundaries. Third, if agents are persuadable, advertising does not disappear but *migrates*: total persuasion spending peaks at 1.68 times the pre-agent advertising budget and remains above it when all buyers are agents. That migration does not chiefly misallocate demand — good sellers still win about 85% of sales either way — but it preserves seller rent (margin 9.9% against 4.0%), cuts consumer net value per unit by about 80%, and concentrates the market by a factor of 2.4 (HHI 7,424 against 3,102). Fourth, we measure the parameter the third result turns on. In a preregistered experiment of 18,000 purchase decisions across three shopping agents, one sentence of unverifiable promotional text raises a mid-ranked product's selection rate by a factor of 8 to 12, while a length-matched neutral placebo does nothing ($z = -0.04$, $+1.18$, $+1.25$) — and the share going to the genuinely best-value product falls on every model. Persuadability is not an assumption of this paper but a measured property of deployed agents. We also read the agentic-commerce protocols standardized between 2025 and 2026 — AP2, ACP, Visa's Trusted Agent Protocol, Mastercard Agent Pay and UCP — and find that they cryptographically secure payment authorization while leaving product claims unstructured and unattested, lowering the friction that governs adoption without touching the parameter that governs who captures the gains. Simulation code, the preregistration, and all 18,000 measured decisions are public.
...
**Contribution.** The paper makes four claims. The first is a share-dependent tipping point in attention rent: we give the condition under which sellers abandon the human-exploiting price (Proposition 1) and show that the switch is discontinuous in the agent share rather than gradual. The second is a migration condition for persuasion spending: we give the condition under which sellers fund agent-directed optimization (Proposition 2), and show numerically that total persuasion spending peaks above the pre-agent advertising budget. A third formal result (Proposition 3) establishes that the mixed population admits no pure-strategy equilibrium in the interior, which explains the cycling that a best-response dynamic exhibits there and is confirmed exactly by the numerics. The third is a reading of the deployed rails: we compare the published agentic-commerce specifications against what an evidence-consuming agent would need, and find that they standardize authorization rather than attestation (§3). The fourth is the resulting claim about distribution: in our model the single parameter with the largest effect on who captures the gains from agent-mediated commerce — seller margin, consumer surplus and market concentration — is not the number of sellers or the cost of search but the agents' resistance to persuasion, which makes an agent's duty of loyalty to its principal an economic mechanism rather than an ethical garnish.

We also name what we do not claim. We do not estimate the persuadability of any deployed agent; our parameter for it is stylized, and we measure it in §8 and discuss what remains in §11. We do not model platform-owned agents with divided loyalties, though we expect that case to strengthen P3. And our numerical results come from a best-response dynamic that reaches a pure-strategy equilibrium in only 12 of 110 runs; we report averages over cycling behavior and treat magnitudes as illustrative and directions as the finding.
...
The threshold $\gamma^*_j$ is strictly decreasing in the twin share $\alpha$, in the margin $p_j - c_j$, and in the choice sharpness $\mu_t$.

The comparative static on $\mu_t$ is the substantive one. The same parameter that makes twin demand winner-take-most — and so drives the margin collapse of Proposition 1 — also raises the marginal return to persuading a twin, because winning a twin's choice wins its entire demand rather than a share of its attention. Sharper agents are both more efficient buyers when they cannot be persuaded and more valuable targets when they can.
...
**Corollary 2.** Writing $T = \alpha M$ and $H = (1-\alpha)M$, conditions (i) and (ii) hold simultaneously for an interval of interior twin shares, and as the grid is refined ($\delta \to 0$) that interval expands to all of $(0,1)$. At the boundaries the tension vanishes: at $\alpha = 0$ there is no segment to undercut for, and at $\alpha = 1$ there are no captive buyers to harvest.

This is the discrete-grid counterpart of the classical non-existence results, and it makes the cycling reported in §6 a prediction rather than a defect. Equilibrium is recovered in mixed strategies — the setting of Dasgupta and Maskin (1986) for games with discontinuous payoffs — which is what the smoothed best-response dynamic of §6 approximates numerically.
...
| Outcome | Regime | α = 0 | α = 0.2 | α = 0.3 | α = 0.5 | α = 1 |
|---|---|---|---|---|---|---|
...
With persuadable agents, agent-directed spend appears as soon as there are agents to address — 3,155 at $\alpha = 0.2$, rising to about 4,800 and staying there — while advertising declines along essentially the same path as in the robust regime. The sum is the result that matters. Total persuasion spending rises from 4,228 in the all-human market to a peak of **7,119** at $\alpha = 0.3$, which is **1.68 times** the pre-agent advertising budget, and settles at 5,102 in the all-agent market, still **1.21 times** the pre-agent level. Sellers spend more to persuade agents than they ever spent to persuade people, in a market where margins have fallen by three quarters.
...
Persuadable agents do **not** meaningfully degrade which sellers win. At $\alpha = 1$ the top-three true-value sellers take 84.5% of sales under robust agents and 85.9% under manipulable ones. A well-informed agent that can be persuaded still buys from good sellers, because agent-directed spending shifts choice at the margin rather than overturning large quality-adjusted price differences.
...
**Consumers capture far less.** Consumer net value per unit at $\alpha = 1$ is +0.23 under manipulable agents against +1.09 under robust ones, a reduction of about 80%. The market allocates to good sellers and then hands most of the resulting surplus to those sellers.
...
Price dispersion also nearly vanishes under manipulable agents (CV 0.050 against 0.098), which is worth flagging because *low dispersion is often read as evidence of competition*. Here it coexists with the most concentrated and least consumer-favorable configuration in the parameter space.
...
The two-regime comparison above fixes $\gamma$ at 0 or 1.5. Figure 2 sweeps the plane, with $\gamma \in \{0, 0.25, 0.5, 0.75, 1, 1.5, 2, 3\}$ against the same adoption path, 4 seeds per cell. Three features of the surface matter more than the two-point contrast.

**Rent at full adoption is monotone and graded in $\gamma$.** Gross margin at $\alpha = 1$ rises steadily with persuadability: 5% at $\gamma = 0$, 6% at 0.25, 8% at 0.5, 10% at 0.75, 12% at 1, 14% at 1.5, 15% at 2, and 18% at 3. There is no threshold here — every increment of persuadability buys the seller side margin it would otherwise lose.

**Concentration responds to very small persuadability.** HHI at $\alpha = 1$ is 3,617 under robust agents and 6,089 at $\gamma = 0.25$ — mild persuadability, the kind that would be hard to detect in an audit, nearly doubles concentration. Beyond that the surface flattens: 8,273 at $\gamma = 1$ and 8,834 at $\gamma = 3$. Most of the structural damage is done by the first small departure from robustness.

**Consumer net value crosses zero.** At full adoption, consumer net value per unit is $+0.83$ under robust agents, $+0.44$ at $\gamma = 0.5$, $+0.18$ at $\gamma = 0.75$, and $-0.24$ at $\gamma = 1$, continuing to fall thereafter. Between $\gamma = 0.75$ and $\gamma = 1$ the fully-agentic market stops delivering consumers more value per unit than the human market it replaced. The level of this series is not meaningful (Appendix C), but the crossing point is: there is a persuadability above which agent adoption is not an improvement for buyers on this measure, and it is not an extreme value of the parameter.
...
In every configuration, persuadable agents leave sellers between 2.2 and 32 times the margin that robust agents leave them, and concentration at full adoption lands between 7,400 and 9,200. The most price-sensitive configuration ($\lambda = 1.4$) produces the starkest contrast, because robust agents drive margin almost to zero there while persuadable ones do not.
...
**Agents are persuadable, and it is persuasion rather than length.** The length placebo is statistically indistinguishable from control on all three models ($z = -0.04$, $+1.18$, $+1.25$), while one sentence of unverifiable promotional text raises the target's choice share from 1.6% to 13.5%, 0.8% to 10.9%, and 2.9% to 35.0% respectively. H1 and H2 are supported on every model. The effect is not a property of one vendor's model.

**The diversion is from the best product, not from a random one.** The sales share of the genuinely best quality-adjusted value product falls under persuasion on every model: 0.563 → 0.507, 0.599 → 0.504, and 0.453 → 0.294. This is the welfare-relevant quantity, and it moves in the direction §5 predicts.
...
### 8.5 Limitations of the measurement

The catalog is synthetic, so the descriptions are ours rather than a real seller's, and real promotional copy is written by people optimizing against real agents. A single sentence is a weak treatment; a seller optimizing seriously would do more. The models are workhorse tiers at default settings, not a deployed shopping product with its own scaffolding, retrieval and system prompt — a real agent may be more robust, or less. And 16 replies from `gemini-3.8-flash` were excluded for not returning a bare product ID; they are spread across arms (7/5/4) so they do not bias the contrast, but a model that narrates instead of answering is a model whose behaviour we are summarizing slightly more crudely than the others.

A further limitation is that our buying instructions are context-free. All five framings ask the agent to choose well; none tells it that the principal is in a hurry, that the purchase matters unusually, or which attribute the principal actually relies upon. Real purchasing contexts carry exactly that information, and there is a specific reason to think it interacts with persuadability rather than merely adding to it: an agent under time pressure has more reason to lean on whatever the listing asserts rather than reconstruct the comparison. If persuadability rises with stated urgency, then agents are least robust in precisely the purchases where a bad choice is hardest to undo, which would be a worse finding than the one we report. The design extends to test this directly — urgency is a framing manipulation, and the matched triad is unchanged — and we have not run it.

None of these push in a known direction. We report the measurement as a lower bound on what a motivated seller could achieve, and as evidence that the parameter is not zero.
...
2. **Equilibrium selection.** Proposition 3 rules out a pure-strategy equilibrium in the interior, so the reported outcomes are averages over the stationary distribution of a smoothed dynamic rather than equilibrium comparative statics. A mixed-strategy characterization of the interior — rather than a numerical approximation to it — remains open.
3. **Stylized parameters.** Magnitudes are illustrative. The directional results (flat-then-cliff margins, migration of persuasion spend, divergence by $\gamma$) are robust across seeds; specific figures such as "1.68 times" are not estimates of any real market.
...
9. **The measurement is context-free.** Susceptibility is estimated under instructions that convey no urgency, no stakes and no attribute priority. Whether γ varies with the principal's circumstances — and in particular whether it rises under urgency — is untested and, if it does, changes the policy implication of §10 substantially.
...
The persuasion spending that attention rent funded does not disappear either. Agents can be moved by how a listing is written — §8 measures it — so the spending migrates to moving agents, and in our simulations it peaks at 1.68 times its pre-agent level. What that migration buys is not a market that picks the wrong sellers — good sellers still win about 85% of sales whether or not agents are persuadable. What it buys is a market where those sellers keep two and a half times the margin, consumers capture roughly 80% less value per unit, and concentration more than doubles. The rent is not dissolved by making buyers smarter; it is relocated, and it flows to whoever the agents can be induced to choose.
...
Dasgupta, P., and Maskin, E. (1986). The Existence of Equilibrium in Discontinuous Economic Games, I: Theory. *Review of Economic Studies*, 53(1), 1–26; and II: Applications, 53(1), 27–41.
...
Madugula, S. S., Esteva de la Rosa, P., and Shankar, D. (2026). *The Dynamic Verifiable Multi-Agent Human Agentic Loyalty Loop (DVM-HALL) Model and the Net Human-Agent Score (NHAS) in Autonomous Commerce*. arXiv:2607.13998.
...
Wadi, A., and Ma, L. (2026). *Does Rank Still Matter? Position Bias When AI Agents Shop on Our Behalf*. arXiv:2608.22697.
...
**Reproduction.** `python3 sim.py 5` writes `results.json` (110 runs); `python3 plot.py` produces `fig_outcomes.png` and the summary table. Every figure in §7 is a mean over the 5 seeds of the final-10-iteration averages recorded in `results.json`.
...
**Inelastic category demand.** The outside option is rarely chosen, so units sold stay close to $M$ in the manipulable regime. The model therefore describes how demand is allocated among sellers, not whether the category expands or contracts. Elastic category demand is the most valuable robustness extension.

**Grid effects.** Prices, advertising and agent spend are chosen from coarse grids. This is what makes the two-candidate structure of Proposition 1 visible, and it also exaggerates the discreteness of the switch relative to a continuous-price model. The knee is a real feature of winner-take-most demand; its sharpness is partly a grid artifact.

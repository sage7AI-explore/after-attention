...
**First, a share-dependent tipping point in attention rent.** We give the condition under which sellers abandon the human-exploiting price (Proposition 1). For a single seller the switch is discontinuous in the agent share rather than gradual; across heterogeneous sellers the aggregate is a sum of such steps, and what the simulation shows at $\Delta\alpha = 0.02$ is a threefold to fourfold steepening of the margin curve over a band about 0.1 wide rather than a jump (Corollary 1, Appendix C).
...
What we cannot do is sign the difference. The allocative effect is **not robustly identified in this model, because its sign depends on the benchmark equilibrium**, and that equilibrium is fixed by two quantities we chose rather than estimated: the smoothing temperature $\tau$ and the agent's logit scale $\mu_t$ (§11 reports an attempt to calibrate the second against our own §8 measurements, and why it does not yet succeed). We therefore report the direction of the rent and concentration effects, which hold their sign at every configuration in the table and in Appendix C, and we decline to sign the allocative one. The headline null should be read as a statement about the headline configuration and nothing more.
...
12. **The allocative effect is not robustly identified, and calibrating the parameter it turns on is harder than it looks.** §7.3 declines to sign that effect because its sign depends on the benchmark equilibrium, which $\mu_t$ and $\tau$ fix. The obvious remedy is to calibrate $\mu_t$ rather than choose it, and our own §8 data appear to offer the means: the fitted $b_{\text{value}}$ of 1.43 to 2.23 measures how sharply four real models' choices track quality-adjusted value, which is what $\mu_t$ represents. We attempted it and report that it does not work as posed. Matching $\mu_t \times \mathrm{SD}(\theta_q q - \lambda p)$ to the measured $b_{\text{value}}$ implies $\mu_t$ of 3.3 to 5.2 at the robust $\alpha = 1$ equilibrium, 5.9 to 9.2 at the persuadable one, and 1.0 to 1.6 in the human-only market, because the dispersion of quality-adjusted value depends on the margin regime — and the margin regime is itself a function of $\mu_t$, so the exercise is circular. It is also a mapping across two different benchmarks: $b_{\text{value}}$ records how choices track the experimenters' value index, not the simulation's. A calibration that worked would need the experiment's catalog and the simulation's to share a value scale by construction, and would need to solve for $\mu_t$ and the equilibrium jointly. Until that is done $\mu_t$ is a chosen parameter. We note one consequence of the attempt even in its failed form: every regime we checked implies a $\mu_t$ in the single digits, and Assumption 2 is the $\mu_t \to \infty$ limit, so that assumption is an idealization our own measurements give no support to.
...
Under robust agents, margin falls from 0.421 to 0.268 across the window. No single step dominates it. The largest, from $\alpha = 0.27$ to $0.29$, is 0.029 — 19% of the total fall against 8% for a uniform slope — and across bootstrap resamples the largest step sits at $\alpha = 0.27$ only 67% of the time, with a ratio of largest step to median step of 2.4 whose bootstrap interval is far too wide to interpret. What the fine grid does establish is a clear change of slope: about 0.2 per unit $\alpha$ below 0.20, about 0.77 between 0.20 and 0.30, and about 0.6 above it. The curve steepens by a factor of three to four over that band and stays steep. Under persuadable agents there is no such feature at this resolution — margin falls from 0.358 to 0.289 in steps of 0.002 to 0.010 — so the steepening is specific to the regime in which Proposition 1's switch operates.

This is what Corollary 1 predicts and we should have said so more carefully. The switch is discontinuous for a single seller; twelve sellers of different quality cross their thresholds at different $\alpha$, and the sum of twelve steps at scattered locations is a knee, not a cliff. The agent-sharpness sweep below is consistent with that mechanism without contradicting it: the largest single step grows from 0.074 to 0.092 to 0.102 as $\mu_t$ rises from 3 to 20, and its location never moves out of $[0.2, 0.3]$, but it never becomes a jump either. We have accordingly changed the language of §1 and §12, which described the aggregate fall as a discontinuity over a narrow band, and we use "tipping point" only in the sense Corollary 1 licenses — a change of slope concentrated where the distribution of $\alpha^*_j$ has mass — rather than in the sense of a discontinuity in the aggregate.

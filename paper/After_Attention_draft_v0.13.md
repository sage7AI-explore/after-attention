...
12. **The allocative effect is not robustly identified, and calibrating the parameter it turns on is harder than it looks.** §7.3 declines to sign that effect because its sign depends on the benchmark equilibrium, which $\mu_t$ and $\tau$ fix. The obvious remedy is to calibrate $\mu_t$ rather than choose it, and our own §8 data appear to offer the means: the fitted $b_{\text{value}}$ of 1.43 to 2.23 measures how sharply four real models' choices track quality-adjusted value, which is what $\mu_t$ represents. Two obstacles stand in the way, and we state them rather than report a number we have not yet earned. The first is a fixed point: $\mu_t$ would have to be matched against $\mathrm{SD}(\theta_q q - \lambda p)$ at the *equilibrium* prices, and those prices are themselves a function of $\mu_t$, so the calibration has to be solved by scanning $\mu_t$ until the implied and assumed values agree rather than evaluated in one pass. That is tractable and we are doing it. The second is not a matter of computation: $b_{\text{value}}$ records how a model's choices track *our experimental* value index over a catalog we wrote, while $\mu_t$ scales quality-adjusted value in a simulated catalog with a different dispersion, so the two are not on a common scale by construction. A calibration that settles $\mu_t$ has to fix that mapping as well as the fixed point. Until both are done $\mu_t$ is a chosen parameter and §7.3's benchmark problem stands. We flag one thing the exercise is likely to bear on whatever it returns: Assumption 2 is the $\mu_t \to \infty$ limit, and nothing we have measured suggests a purchasing agent that sharp.

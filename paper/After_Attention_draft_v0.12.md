...
**Third, migration of persuasion spending, and what it costs.** We give the condition under which sellers fund agent-directed optimization (Proposition 2), and show numerically that total persuasion spending peaks above the pre-agent advertising budget. The cost of that migration is chiefly distributional and structural, and allocative to a degree that rises with how sharp the purchasing agent is: rent stays with sellers and the market concentrates at every agent sharpness we test, while the share reaching the best-value sellers is untouched at our headline sharpness and about ten points lower at a sharper one (§7.3).
...
At the headline agent sharpness, persuadable agents do **not** measurably degrade which sellers win. At $\alpha = 1$ the top-three true-value sellers take 84.5% of sales under robust agents and 85.9% under manipulable ones — a difference of $+1.5$ percentage points against a 95% Monte Carlo interval of $\pm 5.9$, so it is not distinguishable from zero. Under exact best response on the same grid the two figures are 64.2% and 83.9%, so the sign does not even hold across solution concepts (Appendix C).

**That null is a property of $\mu_t = 3$, and it does not survive sharpening the agent.** $\mu_t$ is the twin logit scale — how nearly deterministic the agent's choice is in quality-adjusted price — and 3 is a blunt chooser: the robust market itself misallocates 15% of sales at $\alpha = 1$, which leaves persuadability little room to make things visibly worse. Re-running the full headline sweep at $\mu_t = 10$ and $\mu_t = 20$ (440 runs each, Appendix C) removes that ceiling. Robust agents then allocate essentially perfectly — the top-three share is 0.997 and 1.000 — and the persuadable market falls to 0.875 and 0.909. The contrast is $-12.2$ percentage points ($t = -5.86$) and $-9.1$ ($t = -5.48$), against $+1.5$ ($t = +0.49$) at $\mu_t = 3$. So the honest statement is the conditional one: agent-directed spending shifts choice at the margin rather than overturning large quality-adjusted price differences, and at $\mu_t = 3$ that margin is lost in the noise of a benchmark that is itself imprecise; measured against a sharp agent it costs roughly ten percentage points of allocation.

This cuts against the reassuring reading of our own result rather than for it, and it points the same way as §10: the better purchasing agents get, the more of persuadability's cost shows up as demand going to the wrong sellers rather than only as rent. Everything else in this section strengthens with $\mu_t$ — the margin contrast, the consumer-net-value contrast and the persuasion-migration peak all grow — so §7.3's structural findings are not what is at issue here; the allocative null is.
...
The two regimes differ in exactly one parameter, yet at high agent adoption they produce markets that differ by a factor of 2.5 in margin (9.9% against 4.0%) and a factor of 2.4 in concentration (HHI 7,424 against 3,102; Appendix C shows this ratio is the most configuration-sensitive figure we quote, running from about $1.1\times$ to $3.2\times$), and in which consumer net value per unit falls from $+1.09$ to $+0.23$ — a difference of 0.86 units on a scale whose level is an artifact of the parameterization (Appendix C), so we report the difference and the direction rather than a ratio. Nothing about the number of sellers, their costs, or buyers' underlying preferences differs between the two columns. Who captures the gains from agent-mediated commerce is, in this model, a property of the buyer's agent rather than of the market's structure.
...
Two properties of the estimator are worth stating. First, standard errors are clustered by choice set, which is the level at which the design randomizes. Second, we ran the estimator against 138 simulated null datasets to check its calibration: it returns a one-sided false positive at the 5% level 6.5% of the time, with the standard deviation of $z$ at 1.11 rather than 1.00. The estimator is therefore slightly anti-conservative, though we note the evidence for that is itself marginal: under a correctly sized test, 9 or more rejections in 138 draws has probability $0.25$, and the exact 95% interval on the rejection rate, $[0.030, 0.120]$, contains $0.05$; $\mathrm{sd}(z) = 1.11$ from 138 draws carries a standard error of about $0.067$. The effects in §8.3 have $z$ between 4.9 and 10.1, so the adjustment does not touch them. It does touch §8.5, whose $z$-statistics run from $0.36$ to $3.33$, and we have applied it there rather than only where it is harmless.

**Forty clusters is not many, and we checked what that costs.** The design randomizes at the choice set, of which there are 40, and the cluster-robust sandwich is known to under-state standard errors at that count. We therefore recompute every coefficient with a bias-reduced (CR2) sandwich and Satterthwaite degrees of freedom, following the construction of Pustejovsky and Tipton (2018), and recompute the registered H2 test with a wild cluster bootstrap-$t$ imposing the null (`gamma/robust_se.py`, seeded; it asserts that its point estimates and CR0 standard errors match `estimate.py` to $10^{-6}$ before reporting anything). The correction is small: CR2 inflates standard errors by 1.3% to 2.0% and gives 25 to 38 degrees of freedom. The registered H2 contrast remains significant at one-sided 5% on all four models under both — CR2 $p$ between $2 \times 10^{-6}$ and $3 \times 10^{-5}$, bootstrap $p$ at the resampling floor on every model, with bootstrap 95th percentiles of $t^*$ between 1.68 and 1.77 against the normal 1.645. Against the null datasets above, CR0 rejects about 6% of the time where CR2 and the bootstrap reject about 5%, which locates the anti-conservatism in the sandwich rather than in the specification. Because this is a multinomial choice model there is no residual to reweight, so the bootstrap is the wild *score* bootstrap of Kline and Santos (2012); the implementation is checked against an exact re-solve of the perturbed estimating equations on a subsample of draws.
...
11. **The open-weight replication is one model, on a configuration chosen partly for tractability.** §8.7 establishes that persuasion net of length is not confined to hosted frontier deployments, which is the claim it was registered to test. It does not establish a capability ladder: one 12-billion-parameter model cannot tell us whether susceptibility rises, falls or is flat in model scale, and `gemma4:12b` has both the lowest baseline best-value share of the four models tested and among the largest falls in it, which we cannot disentangle with a single open model. The registration was also amended, before any data, to disable the model's default reasoning mode, because at roughly 1,400 tokens per call a 6,000-call design was infeasible. A model with reasoning disabled is a weaker reconstructor of the quality-adjusted comparison than the same model with it enabled, so §8.7 replicates the triad on that configuration rather than on the model's default one. Three or more open models across a capability range, run in both modes, is the study that would settle what one model cannot.

12. **The allocative result depends on agent sharpness, and we have three points on that curve.** §7.3's null at $\mu_t = 3$ and the $-9$ to $-12$ percentage-point effects at $\mu_t = 10$ and $20$ (Appendix C) bracket a transition we have not mapped. We do not know where between 3 and 10 the effect becomes detectable, whether it is monotone in $\mu_t$, or how it interacts with $\gamma$. A finer ladder in $\mu_t$ swept against persuadability is the single most informative additional simulation we could run, because §10's policy reading turns on which way that surface tilts.
...
The persuasion spending that attention rent funded does not disappear either. Agents can be moved by how a listing is written — §8 measures it on four models, hosted and open — so the spending migrates to moving agents, and in our simulations it peaks at 1.69 times the pre-agent advertising budget. What that migration buys is, at the agent sharpness we headline, not a market that picks the wrong sellers: the share going to the best-value sellers is statistically unchanged. That reprieve is the one result of ours that a better agent removes — sharpen the agent until its robust counterpart allocates perfectly and persuadability costs about ten percentage points of that share. What it buys is a market where those sellers keep two and a half times the margin, concentration more than doubles, and the value consumers capture per unit falls — far enough that, as persuadability rises across the range we sweep, it crosses from positive to negative. The rent is not dissolved by making buyers smarter; it is relocated, and it flows to whoever the agents can be induced to choose.
...
| Smoothing-temperature sweep (Appendix C) | `python3 sim/sim2.py headline --set tau=0.05` and `--set tau=0.3` | `results/results_headline_tau0.05.jsonl`, `results/results_headline_tau0.3.jsonl` (440 runs each) |
| Agent-sharpness sweep (Appendix C) | `python3 sim/sim2.py headline --set mu_t=10` and `--set mu_t=20` | `results/results_headline_mu10.jsonl`, `results/results_headline_mu20.jsonl` (440 runs each) |
...
Two qualifications belong with that. All five fine-grid fixed points sit at $\alpha = 1$ under $\gamma = 0$, where agent demand is deterministic in quality-adjusted price and a pure-strategy profile plainly exists; at $\alpha = 0$ the dynamic does not settle within 40 sweeps on either grid, which is a statement about the iteration budget in a smooth human-only game rather than about existence, and Proposition 3 says nothing about that boundary. And the smoothed dynamic used for the results in §7 does not require convergence at all — it averages over a stationary distribution — so no number in §7 depends on any of this.
...
| Peak persuasion spend / pre-agent budget | $1.69\times$ | $1.92\times$ | $2.03\times$ |
| Robust margin at $\alpha = 0.3$ | 33.1% | 29.9% | 29.2% |

Three of the four headline contrasts strengthen as the agent sharpens. Rent preservation grows ($+0.059$, $+0.072$, $+0.073$), the consumer-net-value gap grows ($-0.87$, $-1.17$, $-1.19$), and persuasion migration grows ($1.69\times$, $1.92\times$, $2.03\times$). The tipping point stays where it is: the robust-agent margin drops from about 39–40% at $\alpha = 0.2$ to 29–33% at $\alpha = 0.3$ at every $\mu_t$.

Two things do not. The **concentration ratio** falls from $2.39\times$ to $1.11\times$, because a sharp robust agent concentrates demand on its own — the robust HHI rises from 3,102 to 8,680 — leaving persuadability little additional room. The absolute levels still differ in the direction we report, and what distinguishes the two markets at high $\mu_t$ is not who wins but what they charge. Taken with the smoothing sweep above, the concentration *ratio* is the most configuration-dependent number in this paper, and "a factor of 2.4" should be read as one point in a range of roughly $1.1\times$ to $3.2\times$ rather than as an estimate.

And the **allocative null of §7.3 reverses**. At $\mu_t = 3$ persuadability changes the top-three share by $+1.5$ percentage points ($t = +0.49$); at $\mu_t = 10$ and $20$ it changes it by $-12.2$ ($t = -5.86$) and $-9.1$ ($t = -5.48$). The reason is visible in the table: at $\mu_t = 3$ the robust benchmark is itself misallocating 15% of sales, and a $1.5$-point effect cannot be seen against a standard deviation of $0.08$. Sharpen the agent until the robust benchmark is essentially perfect and the effect becomes large and precisely estimated. We report the reversal rather than the $\mu_t = 3$ null as the finding, because the null is the weaker measurement of the two, and we note that it is the one result of ours that a *better* purchasing agent makes worse rather than better.

**Grid effects, and one artifact we removed.** Prices, advertising and agent spend are chosen from discrete grids. The coarse grid used in an earlier draft (12 markup points) produced a spurious result: in the fully manipulable market, the top-three true-value sellers appeared to take only 4.9% of sales. On a 16-point grid that figure is 85.9%, and the change does not come from the smoothing — under exact best response on the finer grid it is 83.9% (95% interval $\pm 5.1$, 20 seeds), which an earlier draft reported as 85.4%. With too few price points, one seller outspending on agent-directed optimization can take the whole twin segment and rivals have no fine enough response available. The two-candidate structure behind Proposition 1 is a real feature of winner-take-most demand; the sharpness of any particular discrete outcome is partly the grid.

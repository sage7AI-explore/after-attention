...
Persuadable agents do **not** meaningfully degrade which sellers win. At $\alpha = 1$ the top-three true-value sellers take 84.5% of sales under robust agents and 85.9% under manipulable ones — a difference of $+1.5$ percentage points against a 95% Monte Carlo interval of $\pm 5.9$, so it is not distinguishable from zero, which is the stronger form of the claim. A well-informed agent that can be persuaded still buys from good sellers, because agent-directed spending shifts choice at the margin rather than overturning large quality-adjusted price differences. The claim does not depend on the smoothing, though its arithmetic does: under exact best response on the same grid the two figures are 64.2% and 83.9%, so the persuadable market allocates better rather than indistinguishably, and Appendix C reports why.
...
The two regimes differ in exactly one parameter, yet at high agent adoption they produce markets that differ by a factor of 2.5 in margin (9.9% against 4.0%) and a factor of 2.4 in concentration (HHI 7,424 against 3,102; Appendix C shows this ratio is the most smoothing-sensitive figure we quote), and in which consumer net value per unit falls from $+1.09$ to $+0.23$ — a difference of 0.86 units on a scale whose level is an artifact of the parameterization (Appendix C), so we report the difference and the direction rather than a ratio. Nothing about the number of sellers, their costs, or buyers' underlying preferences differs between the two columns. Who captures the gains from agent-mediated commerce is, in this model, a property of the buyer's agent rather than of the market's structure.
...
The persuasion spending that attention rent funded does not disappear either. Agents can be moved by how a listing is written — §8 measures it on four models, hosted and open — so the spending migrates to moving agents, and in our simulations it peaks at 1.69 times the pre-agent advertising budget. What that migration buys is not a market that picks the wrong sellers — the share going to the best-value sellers is no lower when agents are persuadable than when they are robust. What it buys is a market where those sellers keep two and a half times the margin, concentration more than doubles, and the value consumers capture per unit falls — far enough that, as persuadability rises across the range we sweep, it crosses from positive to negative. The rent is not dissolved by making buyers smarter; it is relocated, and it flows to whoever the agents can be induced to choose.
...
| Smoothing-temperature sweep (Appendix C) | `python3 sim/sim2.py headline --tau 0.05` and `--tau 0.3` | `results/results_headline_tau0.05.jsonl`, `results/results_headline_tau0.3.jsonl` (440 runs each) |

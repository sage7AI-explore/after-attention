...
With persuadable agents, agent-directed spend appears as soon as there are agents to address — 3,155 at $\alpha = 0.2$, rising to about 4,800 and staying there — while advertising declines along essentially the same path as in the robust regime. The sum is the result that matters. Total persuasion spending rises to a peak of **7,119** at $\alpha = 0.3$. Two baselines are available for that comparison and they are not the same number, so we state both. Against the robust regime's all-human advertising budget (4,228) the peak is 1.68 times; against the manipulable regime's own all-human total (4,518) it is **1.58 times**. The second is the like-for-like figure and the one we rely on, for a reason worth making explicit: under $\gamma = 0$ agent-directed spend is constrained to zero rather than chosen, so a robust seller optimizes over a smaller action set than a manipulable one, and the smoothed dynamic has correspondingly less room to fund an instrument with no return. The 304 units of agent-directed spend the manipulable regime records at $\alpha = 0$ — where there are no agents to persuade — is that difference, and Appendix C treats roughly 300 as the noise floor for the spend series. Spending settles at 5,102 in the all-agent market, still 1.13 times the same-regime pre-agent level. Sellers spend more to persuade agents than they ever spent to persuade people, in a market where margins have fallen by three quarters.
...
Two notes on the table. The $\gamma = 0$ configuration constrains agent-directed spend to zero rather than letting a seller choose it, so the robust regime optimizes over a smaller action set than the manipulable one; §7.2 explains what that does to a cross-regime spend comparison. And $\psi$ and $\kappa$ are chosen, not estimated; §7.6 reports a sweep over $\psi$ and states which of its findings survive it.

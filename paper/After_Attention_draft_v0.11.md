...
| Model | H2$'$ persuasion net of length, unhurried cell | H3 change under urgency | H4 change under stakes |
|---|---|---|---|
| `claude-sonnet-5` | $+2.37$ (se 0.84), $z = 2.81$, $p = .003$ | $+0.27$ (se 0.41), $z = 0.67$ — **null** | $-0.27$ (se 0.40), $z = -0.67$ — n.s. |
| `gpt-5.6-terra` | $+2.30$ (se 0.69), $z = 3.33$, $p = .0004$ | $+0.09$ (se 0.26), $z = 0.36$ — **null** | $+1.16$ (se 0.63), $z = 1.84$ — n.s. |

**The persuasion effect replicates** in the unhurried, routine cell on both models, at a size close to the base study.

**Urgency does not detectably change it.** H3 is null on both models under the rule committed before any data was collected. Stated precisely: this rules out an urgency effect of $+1.0$ or larger with 92% power. It does **not** rule out a moderate effect — power is only 54% at $+0.5$ — and it does not show that urgency is irrelevant to how agents buy. It shows that the specific interaction we predicted, at the size that would have mattered, is not there. We report it because we preregistered it, and a null that only gets reported when it is convenient is not a null.

**Stakes are not significant** on either model. A registered secondary test of value $\times$ urgency is nominally significant on one of two models ($p = .048$, uncorrected, no multiplicity correction registered for two models) at about 4% of the value weight; it is reported and not relied upon.
...
Two properties of the estimator are worth stating. First, standard errors are clustered by choice set, which is the level at which the design randomizes. Second, we ran the estimator against 138 simulated null datasets to check its calibration: it returns a one-sided false positive at the 5% level 6.5% of the time, with the standard deviation of $z$ at 1.11 rather than 1.00. The estimator is therefore slightly anti-conservative. The effects we report have $z$ between 4.9 and 10.1, so this does not touch them, but it would matter for a marginal result and we would not want it discovered by someone else.
